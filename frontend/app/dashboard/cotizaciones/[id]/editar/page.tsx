...
export default function EditarCotizacionPage({
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [cliData, prodData, cotData, itemsData] =
                    await Promise.all([
                        listarClientes(),
                        listarProductos(),
                        obtenerCotizacion(cotizacionId),
                        obtenerItemsCotizacion(cotizacionId),
                    ]);
                ...
                // Check if editable
                if (cotData.estado !== "borrador") {
                    setNotEditable(true);
                }

                // Pre‐fill form
                setClienteId(cotData.cliente_id);
                setMoneda(cotData.moneda);
                setVigenciaDias(cotData.vigencia_dias);
                setNotasInternas(cotData.notas_internas ?? "");
    ...
    const removeFromCart = (productoId: number) => {
        setCart((prev) =>
            prev.filter((ci) => ci.producto.id !== productoId)
        );
    };
    ...
    const convertPrice = (precioOriginal: number, monedaProducto: string): number => {
        ...
        if (!tcVenta) return precioOriginal;
        if (moneda === "PEN") {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            await editarCotizacion(cotizacionId, payload);
            ...
            <div className="p-6 md:p-8 max-w-5xl mx-auto space-y-6">
                ...
                <div className="rounded-2xl border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20 p-8 text-center">
                    ...
                    <p className="text-lg font-semibold text-amber-800 dark:text-amber-300">
                        Esta cotización no se puede editar
                    </p>
                    <p className="text-sm text-amber-600 dark:text-amber-400 mt-1">
                        Solo las cotizaciones en estado &quot;borrador&quot; pueden ser
                        editadas.
                    </p>
                    <Link
                        href="/dashboard/cotizaciones"
                        className="mt-4 inline-flex items-center gap-2 rounded-xl bg-amber-600 px-4 py-2 text-sm font-medium text-white hover:bg-amber-700 transition-colors"
                    >
